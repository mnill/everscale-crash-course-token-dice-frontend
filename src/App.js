...
import {useEverWallet} from "./providers/EverWalletProvider/hooks/useEverWallet";
import {Fragment, useEffect, useState} from "react";
import {useTokenWallet} from "./providers/EverWalletProvider/hooks/useTokenWallet";
import {useTokenRoot} from "./providers/EverWalletProvider/hooks/useTokenRoot";
import {useTokenDice} from "./providers/EverWalletProvider/hooks/useTokenDice";
...
function App() {
  ...
  const {isInitializing, hasProvider, selectedNetworkId, account, login, logout} = useEverWallet();
  ...
  const play = async function () {
    ...
    const payload = (await ever.packIntoCell({
      data: {
  ...
  }, [decimals, symbol, firstTx, decodeGameEvents])
  ...
  return (
    <div className="App App-fullscreen">
      ...
      {
        // Something is wrong with EverWallet
        !isInitializing && <Fragment>
          ...
          {
            hasProvider && !account && <div className={'App-center-vertical-horizontal'}>
          ...
          {
            hasProvider && account && targetNetworkId !== selectedNetworkId &&
            <div className={'App-center-vertical-horizontal'}>
              <h2 className={'title'}>
                Please change selected network to "testnet" in EverWallet.
              </h2>
            </div>
          }
        </Fragment>
      ...
      {
        !isInitializing && hasProvider && account && targetNetworkId === selectedNetworkId && <Fragment>
